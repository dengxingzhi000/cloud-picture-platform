...
- RBAC 权限管理：角色/权限 CRUD、用户角色分配、系统保护、审计日志
...
- 图片去重：SHA-256 感知哈希去重已实现，参考计数与清理策略可进一步优化
...
- `V1__init.sql`：合并原 V1~V12 的全部 schema（用户、空间、图片、标签、团队、审核、搜索、协同等）
- `V2__rbac.sql`：RBAC 权限管理基础表（角色、权限、角色-权限关联）
- `V13__rbac_management.sql`：RBAC 审计日志、系统标记、管理端权限

> **历史说明**：原 V2~V12 的增量迁移已合并到 V1，不再逐文件保留。新 schema 变更继续使用新版本脚本，如 `V14__xxx.sql`。
